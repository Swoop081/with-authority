...
const VERSION='v0.9.156-rules-core',MAX_HP=40,HAND_SIZE=5,MAX_MOM=99,DEFAULT_MATCH_TURN_LIMIT=50,MIN_MATCH_TURN_LIMIT=20,MAX_MATCH_TURN_LIMIT=150,MATCH_TURN_LIMIT_STEP=10,STORE='wa-mobile-v0943',BACKUP_STORE='wa-mobile-backup-v0953';
...
function openNextPack(index=0){ensureRewardProfile();const owned=profile.unopenedPacks[index];if(!owned)return boosterHub();const p=(BOOSTERS.products||[]).find(x=>x.id===owned.id);if(!p)return;const pulls=generatePack(p);pulls.forEach(c=>{const key=String(c.id||c.sourceFile||c.originalUNID);profile.collection[key]=(profile.collection[key]||0)+1});profile.unopenedPacks.splice(index,1);profile.openedPacks++;saveProfile();showPackResults(p,pulls)}
...
function exactAutocounterSelection(cards,cost){
  ...
  function visit(pos,total,picked){
    if(total===target){
      const score=picked.reduce((n,x)=>n+scoreCard(x.card,'cpu'),0);
      if(!best||score<best.score)best={score,cards:picked.map(x=>x.card)};
      return;
    }
...
function chooseAutocounterPages(k,cost){
  ...
  if(k==='cpu')return exactAutocounterSelection([...s.hand].sort((a,b)=>scoreCard(a,k)-scoreCard(b,k)),cost);
...
function tryAutocounter(k,attack){
  ...
  const use=k==='cpu'
    ?(s.hand.length>=12||Number(attack.damage||0)>=5||attack.finisher||attack.submission||s.hp<=20)
    :confirm(`${s.name} has no Move counter. Autocounter by ditching ${cost} pages?`);
  if(!use)return false;
  const chosen=chooseAutocounterPages(k,cost);
...
function showCards(){migrateCollectionInventory();const types=['All',...new Set(cards.map(c=>c.cardClass))];const ownedTotal=Object.values(profile.collection||{}).reduce((sum,n)=>sum+Math.max(0,Number(n)||0),0),ownedUnique=cards.filter(c=>collectionCountForCard(c)>0).length;app.innerHTML=`<section class="screen"><div class="topbar"><button class="secondary compact" onclick="home()">Back</button><b>CARD COLLECTION</b><span><b id="collectionVisibleCount">${cards.length}</b> / ${cards.length}</span></div><div class="resultStats collectionStats"><div><b>${ownedTotal}</b><span>Booster pages owned</span></div><div><b>${ownedUnique}</b><span>Unique owned</span></div><div><b>${cards.length}</b><span>Catalogue pages</span></div></div><p class="instruction">Cards won from boosters are shown first with their saved copy count. Unowned pages remain visible for catalogue reference.</p><div class="filterbar">${types.map((t,i)=>`<button class="secondary compact ${i===0?'active':''}" data-type="${esc(t)}">${esc(t)}</button>`).join('')}</div><input id="cardSearch" class="search" placeholder="Search pages..."><div id="cardLibrary" class="library originalCardLibrary"></div></section>`;document.querySelectorAll('.filterbar [data-type]').forEach(btn=>btn.addEventListener('click',()=>{document.querySelectorAll('.filterbar [data-type]').forEach(x=>x.classList.remove('active'));btn.classList.add('active');filterCards(btn.dataset.type)}));document.getElementById('cardSearch')?.addEventListener('input',()=>filterCards(document.querySelector('.filterbar .active')?.dataset.type||'All'));filterCards('All')}
