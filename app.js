...
let ORIGINAL_RUNTIME_MANIFEST={};
let ORIGINAL_RUNTIME=null;
...
const VERSION='v0.9.196',MAX_HP=40,HAND_SIZE=5,MAX_MOM=99,DEFAULT_MATCH_TURN_LIMIT=50,MIN_MATCH_TURN_LIMIT=20,MAX_MATCH_TURN_LIMIT=150,MATCH_TURN_LIMIT_STEP=10,STORE='wa-mobile-v0943',BACKUP_STORE='wa-mobile-backup-v0953';
...
const freshJson=path=>fetch(`${path}?build=0.9.194`,{cache:'no-store',headers:{'Cache-Control':'no-cache'}}).then(r=>{if(!r.ok)throw new Error(`${path} failed to load (${r.status})`);return r.json()});
Promise.all([freshJson('data/demo-cards.json'),freshJson('data/artwork-manifest.json'),freshJson('data/authentic-starter-decks.json'),freshJson('data/starter-roster-map.json'),freshJson('data/booster-products.json'),freshJson('data/original-offline-missions.json'),freshJson('data/original-campaign-v0964.json'),freshJson('data/original-card-script-asts.json'),freshJson('data/original-runtime-manifest.json'),freshJson('data/ai-recommended-decks.json')]).then(([x,a,st,sm,bp,om,oc,asts,runtimeManifest,aiDecks])=>{ART=a;STARTERS=st;STARTER_MAP=sm;BOOSTERS=bp;ORIGINAL_MISSIONS=om;ORIGINAL_CAMPAIGN=oc;ORIGINAL_SCRIPT_ASTS=asts||{};ORIGINAL_RUNTIME_MANIFEST=runtimeManifest||{};ORIGINAL_RUNTIME=typeof WAOriginalRuntime==='function'?new WAOriginalRuntime(ORIGINAL_RUNTIME_MANIFEST):null;AI_DECKS=aiDecks||{decks:[]};cards=x.map(enrichCard);configureSuperstarVariants();showLoginScreen()}).catch(err=>app.innerHTML=`<section class="screen"><div class="logo">LOAD ERROR</div><p>${esc(err.message)}</p></section>`);
...
function superstarDeckRules(key){
  const star=SUPERSTARS[key]||{};
  const base=star.baseKey||key;
  return {key,base,limits:star.momentumMaximums||{}};
}
function cardDeckBlockReason(key,c){
  if(!c||c.unplayable||c.cardClass==='Superstar')return 'This page is not playable in a match playbook.';
  const rules=superstarDeckRules(key);
  if(c.owner&&c.owner!==key&&c.owner!==rules.base)return `Exclusive to ${SUPERSTARS[c.owner]?.name||c.owner}.`;
  const limits=rules.limits;
  const maxFor=type=>Object.prototype.hasOwnProperty.call(limits,type)?Number(limits[type]):99;
  if(c.cardClass==='Momentum'){
    const gains=c.momentumChanges?.gain||{};
    const types=Object.keys(gains).length?Object.keys(gains):[c.momentumType].filter(Boolean);
    if(types.length&&types.every(type=>type!=='Attitude'&&maxFor(type)<=0))return `${SUPERSTARS[key].name} cannot gain ${types.join(' or ')} Momentum.`;
  }
  if((c.cardClass==='Move'||c.cardClass==='Trademark'||c.cardClass==='Finisher')&&c.method&&c.method!=='None'&&maxFor(c.method)<=0)return `${SUPERSTARS[key].name} cannot use ${c.method} moves.`;
  const req=Object.assign({},c.momentumRequirements||{},c.playMomentumRequirements||{});
  for(const [type,n] of Object.entries(req)){
    if(type==='Attitude')continue;
    if(Number(n||0)>maxFor(type))return `Requires ${n} ${type}; ${SUPERSTARS[key].name}'s maximum is ${maxFor(type)}.`;
  }
  return '';
}
function poolFor(key){return cards.filter(c=>c.cardClass!=='Superstar'&&!c.unplayable).filter(c=>!c.owner||c.owner===key||c.owner===(SUPERSTARS[key]?.baseKey||key))}
function deckBuilderPool(key){return poolFor(key).map(c=>({card:c,blockedReason:cardDeckBlockReason(key,c)}))}
...
function totalMomentum(s){return MOM_TYPES.reduce((n,x)=>n+(s.momentum[x]||0),0)+Math.max(0,Number(s.additionalMomentum)||0)}
...
function recordMatchFinish({winner=null,finishType='draw',reason='',finishingMove=null}={}){
  if(!state)return;
  state.winner=winner;
  state.finishType=finishType;
  state.finishReason=reason||'';
  state.finishingMove=finishingMove||state.lastConnectedMove?.name||state.lastMove?.name||null;
  state.finishedTurn=state.round;
}
function finishNonVictoryMatch(reason){
  ...
  state.ended=true;
  recordMatchFinish({winner:null,finishType:'draw',reason,finishingMove:null});
  state.busy=false;
  state.hold=null;
...
function canUseLocationAction(k){
  const s=side(k);
  return !!state&&!state.ended&&!state.busy&&state.control===k&&!state.hold&&!isCurrentlyStunned(k)&&
    !s.specialPlayedThisTurn&&!state.awaitingCpuMoveAck;
}
function originalCanMove(k,location){
  return originalBooleanGate('Can_Move',k,null,{'#location':location,'#superstar':side(k)});
}
function takeItInside(k='player'){
  if(k==='player'&&!claimMatchAction('take-it-inside'))return false;
  if(!canUseLocationAction(k)||!atLocation(k,'Ringside')||!originalCanMove(k,'$InTheRing')){if(k==='player')releaseMatchAction('take-it-inside');return false;}
...
function moveOutside(k='player'){
  ...
  if(!canUseLocationAction(k)||!atLocation(k,'InTheRing')||!originalCanMove(k,'$Ringside')){if(k==='player')releaseMatchAction('move-outside');return false;}
...
function runOriginalCardEvent(event,k,c,extraEnv={}){
  ...
  try{
    const runtimeResult=ORIGINAL_RUNTIME?ORIGINAL_RUNTIME.run({state,page:c,event,execute:()=>interpreter.run(forms,env)}):{output:interpreter.run(forms,env),record:null};
    const result=runtimeResult.output;
    if(runtimeResult.record){state.originalRuntimeAudit=state.originalRuntimeAudit||[];state.originalRuntimeAudit.push(runtimeResult.record);if(state.originalRuntimeAudit.length>1000)state.originalRuntimeAudit.splice(0,state.originalRuntimeAudit.length-1000);}
    ...
    const pageListEntries=(adapter.pageListEntries||[]).map(entry=>({
      list:Number(entry.list)||0,mode:Number(entry.mode)||0,unid:String(entry.page),page:adapter.pageByUNID(entry.page)
    }));
    const pageListUNIDs=pageListEntries.map(entry=>entry.unid);
    const pageList=pageListEntries.map(entry=>entry.page).filter(Boolean);
    return {hadScript:true,executed:true,...result,pageListUNIDs,pageList,pageListEntries};
  }catch(error){
...
function pageIdentityValues(page){return [page?.id,page?.unid,page?.UNID,page?.baseUNID,page?.originalUNID,page?.sourceFile].filter(v=>v!=null).map(String)}
function originalPageListSelection(k,page,event='Get_Page_List'){
  const result=runOriginalCardEvent(event,k,page);
  if(!result.hadScript||!result.executed)return {result,entries:[],ids:new Set(),matches:()=>false};
  const entries=(result.pageListEntries||[]).filter(entry=>entry.page||entry.unid);
  const ids=new Set(entries.flatMap(entry=>[entry.unid,...pageIdentityValues(entry.page)]));
  return {result,entries,ids,matches:c=>pageIdentityValues(c).some(id=>ids.has(id))};
}
function currentSuperstarPage(k){
  const star=currentStar(k),source=String(star?.sourceFile||'');
  return cards.find(page=>page.cardClass==='Superstar'&&String(page.sourceFile||'')===source)||null;
}
function originalDeckChoice(k,page,title,onPick,opts={}){
  const selection=originalPageListSelection(k,page);
  if(!selection.result.hadScript||!selection.result.executed)return false;
  const available=side(k).deck.filter(selection.matches);
  if(!available.length){state.message=`${title}: no eligible pages were found.`;addLog(state.message);render();return true;}
  searchDeckChoice(k,selection.matches,title,onPick,opts.allowCancel!==false);
  return true;
}
function originalHandChoice(ownerKey,page,handOwnerKey,title,onPick){
  const selection=originalPageListSelection(ownerKey,page);
  if(!selection.result.hadScript||!selection.result.executed)return false;
  const hand=side(handOwnerKey).hand.filter(selection.matches);
  if(!hand.length){state.message=`${title}: no eligible pages were found.`;addLog(state.message);render();return true;}
  openChoiceOverlay(ownerKey,title,hand,onPick);
  return true;
}
function originalPlaybookDrawChoice(k,page,title,onPick=()=>{},opts={}){
  const selection=originalPageListSelection(k,page);
  if(!selection.result.hadScript||!selection.result.executed)return false;
  const pool=side(k).deck.filter(selection.matches);
  const finish=pick=>{if(pick){const s=side(k),i=s.deck.indexOf(pick);if(i>=0){s.deck.splice(i,1);s.hand.push(pick);s.pagesDrawn=(s.pagesDrawn||0)+1;addLog(`${s.name} draws ${pick.name} using ${page.name}'s original page list.`)}}onPick(pick||null)};
  if(!pool.length){state.message=`${title}: no eligible pages were found.`;addLog(state.message);render();finish(null);return true;}
  if(k==='cpu'){
    const preferred=typeof opts.prefer==='function'?[...pool].sort((a,b)=>Number(opts.prefer(b)||0)-Number(opts.prefer(a)||0))[0]:pool[0];
    finish(preferred);return true;
  }
  openChoiceOverlay(k,title,pool,finish,{required:opts.allowCancel===true?false:true,source:'original-page-list'});
  return true;
}
function originalCatalogueChoice(k,page,title,onPick=()=>{},opts={}){
  const selection=originalPageListSelection(k,page);
  if(!selection.result.hadScript||!selection.result.executed)return false;
  const seen=new Set(),pool=[];
  for(const entry of selection.entries){const candidate=entry.page;if(!candidate)continue;const key=String(candidate.unid??candidate.id??candidate.sourceFile??candidate.name);if(seen.has(key))continue;seen.add(key);pool.push(candidate)}
  const finish=pick=>onPick(pick||null);
  if(!pool.length){state.message=`${title}: no eligible pages were found.`;addLog(state.message);render();finish(null);return true;}
  if(k==='cpu'){const preferred=typeof opts.prefer==='function'?[...pool].sort((a,b)=>Number(opts.prefer(b)||0)-Number(opts.prefer(a)||0))[0]:pool[0];finish(preferred);return true;}
  openChoiceOverlay(k,title,pool,finish,{required:true,source:'original-catalogue-list'});return true;
}
function originalEventParticipants(k,event,primary=null){
  const seen=new Set(),participants=[];
  const add=(p,owner)=>{if(!p||!originalScriptForms(p,event))return;const key=`${owner||''}:${String(p.sourceFile||p.id||p.name)}`;if(seen.has(key))return;seen.add(key);participants.push({page:p,owner:owner||k})};
  add(primary,k);
  for(const who of [k,other(k)]){
    const s=side(who);
    for(const entry of s.inPlay||[])add(entry.card||entry,who);
    const star=cards.find(p=>p.cardClass==='Superstar'&&String(p.sourceFile||'')===String((SUPERSTARS[s.rosterKey]||{}).sourceFile||''));add(star,who);
  }
  return participants;
}
function dispatchOriginalEvent(event,k,primary=null,extraEnv={}){
  const results=[];
  for(const participant of originalEventParticipants(k,event,primary)){
    const {page,owner}=participant;
    const isPrimary=!!primary&&page===primary;
    if(ORIGINAL_RUNTIME&&!ORIGINAL_RUNTIME.canDispatch(page,event,primary,{isPrimary,owner,actingSide:k}))continue;
    results.push({page,owner,result:runOriginalCardEvent(event,owner,page,{...extraEnv,'#event_owner':side(owner),'#acting_superstar':side(k),'#page':primary||page,'#move':extraEnv['#move']||primary||page})});
  }
  return results;
}
...
const ORIGINAL_PREPLAY_OWN_HAND_TARGETS=new Set(['LariatEX2.gac','Superkick.gac','ChopEX2.gac','SideBackBreakerEX2.gac','RunningClotheslineEX2.gac','RoundKickEX2.gac','KneeDropEX2.gac','LowBlowPunch.gac','NeckbreakerEX2.gac','BackSuplexEX2.gac','SamoanDropEX2.gac','MissileDropkickEX2.gac']);
const ORIGINAL_PREPLAY_OPP_HAND_TARGETS=new Set(['BackRake.gac']);
const ORIGINAL_PREPLAY_OPP_DECK_TARGETS=new Set(['DDTEX3.gac','HipTossEX3.gac','ArmDragTakedownEX3.gac']);
function originalStoredTarget(page){return String(page?.waValues?.User_Data??'')}
function setOriginalStoredTarget(page,pick){
  if(!page||!pick)return false;
  if(!page.waValues)Object.defineProperty(page,'waValues',{value:Object.create(null),writable:true,configurable:true,enumerable:false});
  page.waValues.User_Data=String(pick.unid??pick.UNID??pick.id??pick.sourceFile??'');
  page.originalSelectedTarget={unid:page.waValues.User_Data,name:pick.name||'',sourceFile:pick.sourceFile||''};
  return true;
}
function originalPreplayTargetPool(k,c){
  const f=String(c?.sourceFile||''),selection=originalPageListSelection(k,c);
  if(!selection.result.hadScript||!selection.result.executed)return {selection,pool:[],kind:null};
  const own=side(k),opp=side(other(k));
  if(ORIGINAL_PREPLAY_OWN_HAND_TARGETS.has(f))return {selection,pool:own.hand.filter(x=>x!==c&&selection.matches(x)),kind:'own-hand'};
  if(ORIGINAL_PREPLAY_OPP_HAND_TARGETS.has(f))return {selection,pool:opp.hand.filter(selection.matches),kind:'opponent-hand'};
  if(ORIGINAL_PREPLAY_OPP_DECK_TARGETS.has(f))return {selection,pool:opp.deck.slice(0,3).filter(selection.matches),kind:'opponent-playbook'};
  return {selection,pool:[],kind:null};
}
function prepareOriginalPreplayTarget(k,c,continuePlay){
  const f=String(c?.sourceFile||'');
  if(originalStoredTarget(c)||(!ORIGINAL_PREPLAY_OWN_HAND_TARGETS.has(f)&&!ORIGINAL_PREPLAY_OPP_HAND_TARGETS.has(f)&&!ORIGINAL_PREPLAY_OPP_DECK_TARGETS.has(f)))return false;
  const {pool,kind}=originalPreplayTargetPool(k,c);
  if(!pool.length){state.message=`${c.name}: no eligible original target is available.`;addLog(state.message);state.busy=false;releaseMatchAction('play-card');render();return true;}
  const choose=pick=>{if(!pick){state.busy=false;releaseMatchAction('play-card');render();return}setOriginalStoredTarget(c,pick);addLog(`${side(k).name} chooses ${pick.name} for ${c.name}'s original ${kind} effect.`);continuePlay()};
  if(k==='cpu'){
    let pick;
    if(kind==='own-hand')pick=[...pool].sort((a,b)=>scoreCard(a,k)-scoreCard(b,k))[0];
    else if(kind==='opponent-hand')pick=[...pool].sort((a,b)=>scoreCard(b,other(k))-scoreCard(a,other(k)))[0];
    else pick=[...pool].sort((a,b)=>scoreCard(a,other(k))-scoreCard(b,other(k)))[0];
    choose(pick);return true;
  }
  openChoiceOverlay(k,`${c.name} — choose ${kind==='own-hand'?'a page in your hand':kind==='opponent-hand'?"an opponent's Special":"one of the opponent's next three pages"}`,pool,choose,{required:true,source:'original-preplay-target'});return true;
}
function resolveCard(k,c){if(!state||state.ended)return;const continuePlay=()=>resolveCardPrepared(k,c);if(prepareOriginalPreplayTarget(k,c,continuePlay))return;continuePlay()}
function resolveCardPrepared(k,c){if(c.cardClass==='Momentum')return resolveMomentum(k,c);if(c.latinoHeat)return resolveLatinoHeat(k,c);if(c.cardClass==='Gameplan'||c.cardClass==='Special')return resolveGameplan(k,c);resolveAttack(k,c)}
...
function verifiedSpecialResolve(k,c){const s=side(k),d=side(other(k)),f=String(c.sourceFile||'');
 ...
 if(/^HighSpot\.gac$/i.test(f)){removeCard(k,c);const finish=()=>{s.nextMoveFlags.highSpot=true;done('SEARCH')};if(!originalPlaybookDrawChoice(k,c,'Choose a move costing 6 or more',finish,{prefer:x=>scoreCard(x,k)}))finish();return true}
 ...
 if(/^Hello(?:NWO)?\.gac$/i.test(f)){removeCard(k,c);const finish=()=>{gainMomentum(k,'Attitude',1);done('SEARCH')};if(!originalPlaybookDrawChoice(k,c,'Choose a Special',finish,{prefer:x=>scoreCard(x,k)}))finish();return true}
 ...
 if(/^NipUp\.gac$/i.test(f)){
   ...
   if(!originalPlaybookDrawChoice(k,c,'Kip Up — choose a Move to play this turn',finish,{prefer:x=>scoreCard(x,k)}))finish(null);
   return true;
 }
 if(/^(?:SetUp|WhateverItTakes)\.gac$/i.test(f)){
   removeCard(k,c);const finish=pick=>{if(pick)s.originalTemporaryDraws=(s.originalTemporaryDraws||[]).concat([{instance:pick.instance,sourceFile:f,expiresRound:state.round}]);done('SEARCH')};
   if(!originalPlaybookDrawChoice(k,c,`${c.name} — choose a Move`,finish,{prefer:x=>scoreCard(x,k)}))finish(null);return true;
 }
 if(/^OffTheTopOfTheLadder\.gac$/i.test(f)){
   removeCard(k,c);s.specialPlayedThisTurn=false;
   const finish=pick=>{s.nextMoveFlags.offTopLadder=true;if(pick)s.originalTemporaryDraws=(s.originalTemporaryDraws||[]).concat([{instance:pick.instance,sourceFile:f,expiresRound:state.round}]);done('SEARCH')};
   if(!originalPlaybookDrawChoice(k,c,`${c.name} — choose a High Risk Move`,finish,{prefer:x=>scoreCard(x,k)}))finish(null);return true;
 }
 if(/^BreakTheWallsDown\.gac$/i.test(f)){
   removeCard(k,c);const finish=()=>done('SEARCH');
   if(!originalPlaybookDrawChoice(k,c,`${c.name} — choose a Defensive Move`,finish,{prefer:x=>scoreCard(x,k)}))finish(null);return true;
 }
 if(/^RingVeteranEX2\.gac$/i.test(f)){
   removeCard(k,c);const finish=pick=>{if(pick){s.inPlay.push({name:c.name,sourceFile:f,card:c,cardClass:c.cardClass,duration:999,ringVeteran:true,bannedPage:pick.name});s.playedRingVeteran=(s.playedRingVeteran||0)+1;addLog(`${s.name} names ${pick.name} with Ring Veteran; that page cannot be played.`)}done('IN PLAY')};
   if(!originalCatalogueChoice(k,c,`${c.name} — name a Special that cannot be played`,finish,{prefer:x=>scoreCard(x,k)}))finish(null);return true;
 }
 return false}
...
function tryAutocounter(k,attack){
  ...
  if(!originalAutocounterAllowed(k,attack)){addLog(`${side(k).name} cannot Autocounter ${attack.name} because an original effect prevents it.`);return false;}
  const s=side(k),cost=autocounterCost(k,attack);
  ...
  if(k==='cpu'){
    ...
    const originalDecision=originalAiAutoCounterDecision(k,attack);const emergency=(Number(attack.damage)||0)>=s.hp||attack.finisher&&s.hp<=30||attack.submission&&s.hp<=18;
    const hasConcreteFollowUp=!!plan&&(plan.immediatelyAffordableMoves.length>0||plan.oneMomentumFollowUps.length>0);
    if(originalDecision===false)return false;if(originalDecision!==true&&!emergency&&(!plan||plan.quality<12||!hasConcreteFollowUp||plan.retained.length<2)){
  ...
  state.autoCounterSummary={
    ...
    discardedNames,
    bonusDraw,
    originalAutoDitchScores:chosen.map(card=>({name:card.name,score:originalAutoDitchScore(card,k)}))
  };
...
function resolveCounterWindow(attacker,card,depth){const defender=other(attacker),a=side(attacker),d=side(defender);const illegal=String(card.modifiers||'').toLowerCase().includes('illegal');const plead=d.hand.find(x=>x.pleadReferee)&&!d.inPlay.some(e=>e.name==='Ref Bump')&&d.alignment!=='Heel';if(illegal&&plead){removeCard(defender,plead);addWarnings(attacker,4,'Plead to the Referee');d.inPlay=d.inPlay.filter(e=>e.name!=='Distract The Referee');state.pile={card:plead,owner:defender,status:'REFEREE APPEAL'};addLog(`${d.name} plays Plead to the Referee; ${a.name} receives 4 Warnings.`)}const calling=a.pendingCallingForIt;const dstar=currentStar(defender);if(d.superKey==='bigshow'&&/2E/i.test(dstar.sourceFile||'')&&state.round-(d.gMoneyLastTurn||-99)>=10){const use=defender==='cpu'?(Number(card.damage)||0)>=4:confirm(`Use G-Money to reduce ${card.name} damage by 4?`);if(use){card={...card,damage:Math.max(0,(Number(card.damage)||0)-4)};d.gMoneyLastTurn=state.round;addLog(`${d.name} uses G-Money: ${card.name} does -4 damage.`)}}const counter=calling?null:findCounter(defender,card);if(!counter&&!calling&&tryAutocounter(defender,card)){
  certifyNoPartialCounterResolution(state.counterWindowSnapshot,attacker,card,'Autocounter');
  state.proposedMove=null;state.counterWindowSnapshot=null;
  a.hitStreak=0;
  a.impactStreak=0;
...
applySourcePinModifiers(k,c);if(c.face)a.alignment='Face';if(c.heel){a.alignment='Heel';if(d.superKey==='flair'){gainMomentum(dKey,'Attitude',3);addLog(`${d.name} gains 3 Attitude because a Heel page was played.`)}}if(a.superKey==='austin'&&c.attachedDamageSpecial){drawPages(k,1);addLog(`${a.name} draws a page because a Damage Special accompanied a connected move.`)}if(!originalScriptForms(c,'Move_Connected')){if(c.drawEffect){const before=a.hand.length;drawPages(k,c.drawEffect);const drawn=a.hand.length-before;if(drawn)addLog(`${c.name} effect: ${a.name} draws ${drawn} page${drawn===1?'':'s'}.`)}if(c.discardOpponent&&d.hand.length){for(let n=0;n<c.discardOpponent&&d.hand.length;n++){const dropped=d.hand.splice(Math.floor(Math.random()*d.hand.length),1)[0];d.discard.push(dropped);addLog(`${d.name} discards ${dropped.name}.`)}}if(c.discardTwoIfFour&&d.hand.length>=4){for(let n=0;n<2&&d.hand.length;n++){const dropped=d.hand.splice(Math.floor(Math.random()*d.hand.length),1)[0];d.discard.push(dropped);addLog(`${d.name} ditches ${dropped.name}.`)}}if(c.discardNonMomentum){const choices=d.hand.filter(x=>x.cardClass!=='Momentum');if(choices.length){const dropped=choices[Math.floor(Math.random()*choices.length)],i=d.hand.indexOf(dropped);d.hand.splice(i,1);d.discard.push(dropped);addLog(`${d.name} ditches non-Momentum page ${dropped.name}.`)}}if(c.discardAllStrength&&d.zoneDamage.Body>=7){const dropped=d.hand.filter(x=>x.method==='Strength');d.hand=d.hand.filter(x=>x.method!=='Strength');d.discard.push(...dropped);if(dropped.length)addLog(`${d.name} ditches ${dropped.length} Strength move(s).`)}const millCount=Math.max(Number(c.millOpponent)||0,mods.includes('$mill')?1:0);if(millCount){for(let n=0;n<millCount&&d.deck.length;n++){const milled=d.deck.shift();d.discard.push(milled);addLog(`${c.name} effect: ${d.name}'s next playbook page, ${milled.name}, is ditched.`)}if(!d.deck.length&&millCount)addLog(`${d.name}'s playbook is now empty.`)}if(c.opponentAttitudeLoss)spendMomentum(dKey,c.opponentAttitudeLoss,'Attitude');if(c.opponentAttitudeGain)gainMomentum(dKey,'Attitude',c.opponentAttitudeGain);if(c.choiceDitchSpecialOrAttitude){const pool=d.hand.filter(x=>x.cardClass==='Special');if(pool.length){const drop=pool[Math.floor(Math.random()*pool.length)];d.hand.splice(d.hand.indexOf(drop),1);d.discard.push(drop);addLog(`${d.name} ditches Special page ${drop.name}.`)}else gainMomentum(k,'Attitude',1)}if(c.moveSelfRingside)state.position='Ringside';if(c.treeOfWoe&&String(c.modifiers||'').includes('Impact')){damage+=7;d.stun=Math.max(d.stun,2)}if(c.discardSelf){for(let n=0;n<c.discardSelf&&a.hand.length;n++)a.discard.push(a.hand.pop())}if(c.recoverEffect&&a.discard.length){a.hand.push(a.discard.pop());a.recoveries++}
  ...
  dispatchOriginalEvent('Move_Connected',k,c,{'#move':c,'#target':d});
  // Certified effects for the three competitive decks.

 if(/^KickToGut\.gac$/i.test(String(c.sourceFile||''))){
   const listResult=runOriginalCardEvent('Get_Page_List',k,c);
   const eligibleIds=new Set((listResult.pageListUNIDs||[]).map(String));
   const pred=x=>eligibleIds.has(String(x.unid??x.UNID??x.id??x.sourceFile??''))||eligibleIds.has(String(x.baseUNID??x.originalUNID??''));
   if(k==='cpu'){
     const opts=a.deck.filter(pred),pick=opts.find(x=>/Stone Cold Stunner/i.test(x.name))||opts[0];
     if(pick){a.deck.splice(a.deck.indexOf(pick),1);a.hand.push(pick);a.pagesDrawn++;addLog(`${a.name} draws ${pick.name} with ${c.name}'s original page list.`)}
   }else searchDeckChoice(k,pred,`${c.name} — choose an eligible page`,()=>{},true);
 }
 // Compatibility-only fallback: TheRocksDDT2E.gac has no recovered Move_Connected handler.
 if(/^TheRocksDDT2E\.gac$/i.test(String(c.sourceFile||''))){if((a.momentum.Knowledge||0)>0){drawPages(k,1);gainMomentum(k,'Attitude',1)}if((a.momentum.Agility||0)>0){drawPages(k,1);gainMomentum(k,'Attitude',1)}}
 a.lastConnectedMethod=c.method;if(c.setsRingside)state.position='Ringside';else if(c.putOnMat)state.position='Grounded';else state.position=c.setsPosition==='Same'?state.position:(c.setsPosition||'Standing');if(c.attachedCrowdAttitude){gainMomentum(k,'Attitude',1);addLog(`${a.name} gains 1 Attitude from Crowd Support.`)}if(c.attachedSpecialStun)applyCertifiedStun(dKey,c.attachedDamageSpecial||c,Number(c.attachedSpecialStun)||0,k);if(c.stun&&!/\(WAStun\s+#target/.test(originalScriptFor(c,'Move_Connected')))applyCertifiedStun(dKey,c,Number(c.stun)||0,k);if(c.conditionalStun&&!isCurrentlyStunned(dKey)&&d.hand.length<a.hand.length)applyCertifiedStun(dKey,c,1,k);changeControlCertified(k,'after the connected move');state.pile={card:c,owner:k,status:'SUCCESS'};if(c.finisher)state.lastFinisher={by:k,round:state.round};addLog(`${a.name}'s ${c.name} succeeds for ${damage} damage to ${zone}.`);gainMomentum(k,'Attitude',1);addLog(`${a.name} gains 1 Attitude for hitting with ${c.name}.`);const defenderAttitudeBefore=d.momentum.Attitude||0;d.momentum.Attitude=Math.max(0,defenderAttitudeBefore-1);if(defenderAttitudeBefore>0)addLog(`${d.name} loses 1 Attitude after being hit by ${c.name}.`);state.message=`SUCCESS! ${c.name} lands for ${damage} ${zone} damage.`;const endMoveRows=dispatchOriginalEvent('End_Move',k,c,{'#move':c,'#target':d});for(const who of ['player','cpu'])if(Number(side(who).additionalMomentum||0)>0&&!side(who).inPlay.some(e=>/^DrawFromTheFans\.gac$/i.test(String(e.sourceFile||e.card?.sourceFile||''))))side(who).additionalMomentum=0;state.lastOriginalMoveLifecycleAudit={round:state.round,move:c.name,sourceFile:c.sourceFile||'',beginMoveHandlers:Number(state.lastOriginalMoveLifecycleAudit?.beginMoveHandlers)||0,endMoveHandlers:endMoveRows.filter(row=>row.result?.hadScript&&row.result?.executed).length,stunned:isCurrentlyStunned(dKey),control:state.control};if(c.submission)return beginSubmission(k,c);if(k==='cpu'){state.awaitingCpuMoveAck=true;state.busy=false;state.message+=` Press PASS to continue.`;render();return}finishResolution()}
...
function end(win,reason){
  ...
  const winnerKey=win?'player':'cpu';
  const lower=String(reason||'').toLowerCase();
  const finishType=lower.includes('submission')||lower.includes('submits')?'submission':lower.includes('count-out')||lower.includes('count out')?'count-out':lower.includes('disqual')?'disqualification':lower.includes('pinfall')||lower.includes('pinned')?'pinfall':'win';
  state.ended=true;
  recordMatchFinish({winner:winnerKey,finishType,reason:reason||`${side(winnerKey).name} wins the match`,finishingMove:state.lastConnectedMove?.name||state.lastMove?.name||null});
  state.busy=false;
  state.hold=null;
  state.awaitingCpuMoveAck=false;state.awaitingAutoCounterAck=false;state.pendingAutoCounterControl=null;state.autoCounterSummary=null;
...
else if(s.superKey==='jericho'){
  const page=currentSuperstarPage(k);
  if(/2E/i.test(star.sourceFile||'')){
    if(s.abilityUses>=2)return;
    const picked=page&&originalDeckChoice(k,page,'King of the World — choose a Momentum page',c=>{
      s.momentum.Attitude=0;s.abilityUses++;
      finishAbilityChoice(k,`King of the World draws ${c.name} (${s.abilityUses}/2).`);
    });
    if(picked)return;
    searchDeckChoice(k,c=>c.cardClass==='Momentum','King of the World — choose a Momentum page',c=>{s.momentum.Attitude=0;s.abilityUses++;finishAbilityChoice(k,`King of the World draws ${c.name} (${s.abilityUses}/2).`)});return;
  }else{
    if(s.abilityUses>=3||(s.momentum.Attitude||0)<1)return;
    const picked=page&&originalDeckChoice(k,page,'Jerichoholics — choose a Momentum page',c=>{
      s.momentum.Attitude--;s.abilityUses++;
      finishAbilityChoice(k,`Jerichoholics draws ${c.name} (${s.abilityUses}/3).`);
    });
    if(picked)return;
    searchDeckChoice(k,c=>c.cardClass==='Momentum','Jerichoholics — choose a Momentum page',c=>{s.momentum.Attitude--;s.abilityUses++;finishAbilityChoice(k,`Jerichoholics draws ${c.name} (${s.abilityUses}/3).`)});return;
  }
}
...
else if(s.superKey==='bookert'){
  if(s.abilityUses>=9)return;
  const page=currentSuperstarPage(k);
  const picked=page&&originalHandChoice(k,page,k,'Can You Dig It? — ditch a Move Page',c=>{
    const i=s.hand.indexOf(c);if(i>=0){s.hand.splice(i,1);s.discard.push(c)}
    drawPages(k,1);s.abilityUses++;
    finishAbilityChoice(k,`Can You Dig It? ditches ${c.name} and draws a page (${s.abilityUses}/9).`);
  });
  if(picked)return;
  ditchChoice(k,c=>isMovePage(c),'Can You Dig It? — ditch a Move Page',c=>{drawPages(k,1);s.abilityUses++;finishAbilityChoice(k,`Can You Dig It? ditches ${c.name} and draws a page (${s.abilityUses}/9).`)});return;
}
...
else if(s.superKey==='bubba'){
  if(s.bubbaUses>=2)return;const page=currentSuperstarPage(k);
  const picked=page&&originalDeckChoice(k,page,'Those Damn Dudleyz! — choose an Event or Unique page',c=>{s.bubbaUses++;finishAbilityChoice(k,`Those Damn Dudleyz! draws ${c.name} (${s.bubbaUses}/2).`);});
  if(picked)return;
  searchDeckChoice(k,c=>c.cardClass==='Special'&&(/event/i.test(String(c.modifiers||''))||/unique/i.test(String(c.modifiers||c.description||''))),'Those Damn Dudleyz! — choose an Event or Unique page',c=>{s.bubbaUses++;finishAbilityChoice(k,`Those Damn Dudleyz! draws ${c.name} (${s.bubbaUses}/2).`)});return;
}
else if(s.superKey==='dvon'){
  if(s.dvonUses>=2)return;const page=currentSuperstarPage(k);
  const picked=page&&originalDeckChoice(k,page,'D-Von! Get the... — choose an eligible page',c=>{s.dvonUses++;finishAbilityChoice(k,`D-Von! Get the... draws ${c.name} (${s.dvonUses}/2).`);});
  if(picked)return;
  searchDeckChoice(k,c=>/weapon|ringside|set up a table|set up a ladder/i.test(`${c.modifiers||''} ${c.description||''} ${c.name||''}`),'D-Von! Get the... — choose an eligible page',c=>{s.dvonUses++;finishAbilityChoice(k,`D-Von! Get the... draws ${c.name} (${s.dvonUses}/2).`)});return;
}
else if(s.superKey==='hurricane'){
  if(s.hurricaneAbilityUsed||state.round>=15)return;
  const opp=side(other(k)),page=currentSuperstarPage(k);
  const pickBorrowed=c=>{const i=opp.hand.indexOf(c);if(i<0)return;opp.hand.splice(i,1);s.hand.push(c);const reqs=c.playMomentumRequirements||c.momentumRequirements||{},gained={};for(const [type,n] of Object.entries(reqs)){const need=Math.max(0,(Number(n)||0)-(s.momentum[type]||0));if(need){gainMomentum(k,type,need);gained[type]=need}}const flex=Math.max(0,actualCost(c,k)-totalMomentum(s));if(flex){gainMomentum(k,'Attitude',flex);gained.Attitude=(gained.Attitude||0)+flex}s.hurricaneBorrowedMomentum=gained;s.hurricaneAbilityUsed=true;s.abilityUsed=true;finishAbilityChoice(k,`Super Powers steals ${c.name} and grants enough temporary Momentum to play it.`)};
  if(page&&originalHandChoice(k,page,other(k),'Super Powers — choose an opponent move',pickBorrowed))return;
  openChoiceOverlay(k,'Super Powers — choose an opponent move',opp.hand.filter(isMovePage),pickBorrowed);return;
}
else if(s.superKey==='scotty'){
  if(s.scottyUses>=4||s.hitStreak<2||s.scottyUsedThisString)return;const page=currentSuperstarPage(k);
  const picked=page&&originalDeckChoice(k,page,'Turn it up! — choose a Gameplan Special',c=>{s.scottyUses++;s.scottyUsedThisString=true;finishAbilityChoice(k,`Turn it up! draws ${c.name} (${s.scottyUses}/4).`);});
  if(picked)return;
  searchDeckChoice(k,c=>c.cardClass==='Gameplan'||(c.cardClass==='Special'&&/gameplan/i.test(String(c.modifiers||c.description||''))),'Turn it up! — choose a Gameplan Special',c=>{s.scottyUses++;s.scottyUsedThisString=true;finishAbilityChoice(k,`Turn it up! draws ${c.name} (${s.scottyUses}/4).`)});return;
}
...
else if(s.superKey==='xpac'){
  if(s.abilityUsed)return;const page=currentSuperstarPage(k);
  const picked=page&&originalDeckChoice(k,page,'Martial Arts Pro — choose a Leg Extended move',c=>{s.abilityUsed=true;finishAbilityChoice(k,`Martial Arts Pro draws ${c.name}.`);});
  if(picked)return;
  searchDeckChoice(k,c=>isMovePage(c)&&/leg extended/i.test(String(c.originalMoveType||c.description||'')),'Martial Arts Pro — choose a Leg Extended move',c=>{s.abilityUsed=true;finishAbilityChoice(k,`Martial Arts Pro draws ${c.name}.`)});return;
}
...
function originalAiPinDecision(k,score){
  const star=currentSuperstarPage(k);if(!star)return null;
  const r=runOriginalCardEvent('AI_PinTarget',k,star,{'#target':side(other(k)),'#score':Number(score)||0});
  return r.hadScript&&r.executed?!!r.result:null;
}
function originalAiAutoCounterDecision(k,attack){
  const star=currentSuperstarPage(k);if(!star)return null;
  const r=runOriginalCardEvent('AI_AutoCounter',k,star,{'#move':attack,'#target':side(k),'#score':Number(attack?.damage)||0});
  return r.hadScript&&r.executed?!!r.result:null;
}
...
function deckValidation(key,ids){
  const pages=ids.map(cardById).filter(Boolean),errors=[];
  if(ids.length<55)errors.push(`Add ${55-ids.length} more pages.`);
  if(ids.length>80)errors.push(`Remove ${ids.length-80} pages.`);
  const blocked=pages.filter(c=>cardDeckBlockReason(key,c));
  if(blocked.length)errors.push(`${blocked.length} page${blocked.length===1?' is':'s are'} incompatible with this Superstar.`);
  const counts={};for(const c of pages){const g=canonicalCountName(c);counts[g]=(counts[g]||0)+1}
  const over=Object.entries(counts).filter(([,n])=>n>5);if(over.length)errors.push(`${over.length} card group${over.length===1?' exceeds':'s exceed'} the five-copy limit.`);
  return {legal:errors.length===0,errors,pages};
}
function deckBuilder(key='player',query='',filter='all'){
  selectedSuperstar=key;
  if(!profile.decks[key])profile.decks[key]=[...defaultDeckIds(key)];
  const ids=profile.decks[key];
  const q=String(query||'').trim().toLowerCase();
  const entries=deckBuilderPool(key).filter(({card})=>!q||[card.name,card.cardClass,card.method,card.description,card.sourceFile].join(' ').toLowerCase().includes(q)).filter(({card,blockedReason})=>filter==='all'||(filter==='legal'&&!blockedReason)||(filter==='blocked'&&blockedReason)||(filter===card.cardClass));
  const validation=deckValidation(key,ids);
  const counts={};for(const id of ids){const c=cardById(id);if(c){const g=canonicalCountName(c);counts[g]=(counts[g]||0)+1}}
  const summary=validation.legal?'LEGAL PLAYBOOK':validation.errors.join(' ');
  app.innerHTML=`<section class="screen deckBuilderScreen"><div class="topbar"><button class="secondary compact" onclick="playbookHub()">Back</button><b>${esc(SUPERSTARS[key].name)} DECK BUILDER</b><span>${ids.length}/80</span></div><p class="instruction">Build a 55–80 page playbook. Cards are limited to five copies. Moves and Momentum this Superstar cannot use remain visible but are locked.</p><div class="deckSummary"><div><b class="${validation.legal?'win':'warning'}">${esc(summary)}</b><p>${ids.length} pages · ${Object.keys(counts).length} different cards</p></div><button class="secondary compact" onclick="resetDeck('${key}')">Reset Starter</button><button class="secondary compact" onclick="clearCustomDeck('${key}')">Clear</button><button class="primary compact" ${validation.legal?'':'disabled'} onclick="saveDeck('${key}')">Save Playbook</button></div><div class="cardTools"><input id="deckSearch" type="search" placeholder="Search cards..." value="${esc(query)}"><select id="deckFilter"><option value="all" ${filter==='all'?'selected':''}>All cards</option><option value="legal" ${filter==='legal'?'selected':''}>Usable only</option><option value="blocked" ${filter==='blocked'?'selected':''}>Blocked</option>${['Momentum','Move','Trademark','Finisher','Action','Support','Gameplan','Entrance','Special','Counter','Autocounter'].map(x=>`<option value="${x}" ${filter===x?'selected':''}>${x}</option>`).join('')}</select></div><div class="library">${entries.map(({card:c,blockedReason})=>{const group=canonicalCountName(c),n=counts[group]||0,limit=5,blocked=!!blockedReason,full=n>=limit||ids.length>=80;return`<article class="card class-${String(c.cardClass||'special').toLowerCase()} ${blocked?'unsupported':''}"><div class="cardhead"><h3>${esc(c.name)}</h3><span class="tag">${n} / ${limit}</span></div><div class="position">${esc(c.cardClass)}${c.method?` · ${esc(c.method)}`:''}${c.position?` · ${esc(c.position)} → ${esc(c.setsPosition)}`:''}</div><p>${esc(c.description||'')}</p>${blocked?`<p class="warning"><b>BLOCKED:</b> ${esc(blockedReason)}</p>`:''}<div class="stats"><span>DMG ${Number(c.damage||0)}</span><span>${c.cardClass==='Momentum'?esc(c.momentumType||'Momentum')+' +'+(c.momentumAmount||1):'COST '+Number(c.momentumCost||0)}</span><span>${esc(c.originalSet||c.variantEdition||'Original')}</span></div><div class="qty"><button onclick="changeDeck('${key}','${c.id}',-1)" ${n===0?'disabled':''}>−</button><b>${n}</b><button onclick="changeDeck('${key}','${c.id}',1)" ${blocked||full?'disabled':''}>+</button></div></article>`}).join('')}</div></section>`;
  document.getElementById('deckSearch')?.addEventListener('input',e=>deckBuilder(key,e.target.value,document.getElementById('deckFilter')?.value||'all'));
  document.getElementById('deckFilter')?.addEventListener('change',e=>deckBuilder(key,document.getElementById('deckSearch')?.value||'',e.target.value));
}
function changeDeck(key,id,delta){
  const ids=profile.decks[key]||[];
  if(delta<0){const c=cardById(id),group=c&&canonicalCountName(c);const i=ids.findIndex(x=>{const d=cardById(x);return d&&canonicalCountName(d)===group});if(i>=0)ids.splice(i,1)}
  else{const c=cardById(id),reason=cardDeckBlockReason(key,c);if(reason){alert(reason);return}const group=canonicalCountName(c),n=ids.map(cardById).filter(x=>x&&canonicalCountName(x)===group).length;if(n<5&&ids.length<80)ids.push(id)}
  profile.decks[key]=ids;saveProfile();deckBuilder(key,document.getElementById('deckSearch')?.value||'',document.getElementById('deckFilter')?.value||'all')
}
function saveDeck(key){const validation=deckValidation(key,profile.decks[key]||[]);if(!validation.legal){alert(validation.errors.join('\n'));return}saveProfile();selectedSuperstar=key;selectedDeckMode='custom';selectedRecommendedDeck=null;deckChoiceScreen(key)}
function resetDeck(key){profile.decks[key]=defaultDeckIds(key);saveProfile();deckBuilder(key)}
function clearCustomDeck(key){if(confirm('Clear every page from this custom playbook?')){profile.decks[key]=[];saveProfile();deckBuilder(key)}}
...
function deckChoiceScreen(key){
 ...
 ensureUnlockedRecommendations();
 const st=starterStatus(key);
 const base=SUPERSTARS[key]?.baseKey||key;
 const exactRecommendations=(AI_DECKS.decks||[]).filter(d=>d.unlockedTestDeck&&d.superstar===key&&!d.starterUpgrade);
 const recommendations=exactRecommendations.length?exactRecommendations:(AI_DECKS.decks||[]).filter(d=>d.unlockedTestDeck&&d.superstar===base&&!d.starterUpgrade);
 const recommendedHtml=recommendations.map(rec=>`<article class="deckChoiceCard aiChoice">${artImg(starArt(key),'entityPortrait',SUPERSTARS[key].name)}<h2>${esc(rec.finisherName||'FINISHER')} / ${esc(rec.trademarkName||'TRADEMARK')}</h2><p>${esc(rec.strategy)}</p><p><b>${rec.size}</b> pages · ${esc((rec.keyMoveTypes||[]).join(' / ')||'focused move types')}</p><div class="deckChoiceActions"><button class="primary compact" type="button" onclick="selectRecommendedDeck('${esc(rec.id)}')">USE RECOMMENDED</button><button class="secondary compact" type="button" onclick="inspectRecommendedDeck('${esc(rec.id)}')">INSPECT</button></div></article>`).join('');
 app.innerHTML=`<section class="screen originalPanelScreen"><div class="topbar"><button class="secondary compact" onclick="superstarSelect()">Back</button><b>${esc(SUPERSTARS[key].name)}</b><span>${st.total} STARTER PAGES</span></div><div class="deckChoiceGrid"><article class="deckChoiceCard starterChoice">${artImg(starArt(key),'entityPortrait',SUPERSTARS[key].name)}<h2>AUTHENTIC STARTER</h2><p>${st.starter?.starterType==='official-product-build'?'Historically grounded Official Product Build.':'Exact recovered starter with its original Lead Off five and card order.'}</p><p><b>${st.mapped}/${st.total}</b> pages resolved</p><button class="secondary compact" type="button" onclick="selectStarterDeck()" ${st.ready?'':'disabled'}>USE STARTER</button></article>${recommendedHtml||'<article class="deckChoiceCard"><h2>RECOMMENDATION UNAVAILABLE</h2><p>No focused deck was generated for this edition.</p></article>'}<article class="deckChoiceCard customChoice">${artImg(starArt(key),'entityPortrait',SUPERSTARS[key].name)}<h2>CUSTOM PLAYBOOK</h2><p>${profile.decks?.[key]?.length?`${profile.decks[key].length} saved pages. Build, edit or use your own legal playbook.`:'Create a new playbook from every card this Superstar can legally use.'}</p><div class="deckChoiceActions"><button class="primary compact" type="button" onclick="selectCustomDeck()" ${deckValidation(key,profile.decks?.[key]||[]).legal?'':'disabled'}>USE CUSTOM</button><button class="secondary compact" type="button" onclick="deckBuilder('${key}')">${profile.decks?.[key]?.length?'EDIT':'BUILD'}</button></div></article></div></section>`;
}
...
window.__WA_TEST__={
  ...
  createSide,
  resolveCard,
  originalPageListSelection,
  originalPreplayTargetPool,
  setOriginalStoredTarget,
  ...
  maintainSubmission,
  releaseSubmission,
  getOriginalRuntimeAudit:()=>ORIGINAL_RUNTIME?[...ORIGINAL_RUNTIME.records]:[],
  counterOptions,
  originalAutocounterAllowed,
  autocounterCost,
  originalAutoDitchScore,
  exactAutocounterSelection,
  tryAutocounter,
  resolveCounterWindow,
  getCounterCertification:()=>state?{costAudit:[...(state.autoCounterCostAudit||[])],counterAudit:[...(state.counterCertification||[])],failures:[...(state.certificationFailures||[])]}:null,
  getOriginalDamageAudit:()=>state?.lastOriginalDamageAudit||null
};
