...
const VERSION='v0.9.137-rules-core',MAX_HP=40,HAND_SIZE=5,MAX_MOM=99,DEFAULT_MATCH_TURN_LIMIT=50,MIN_MATCH_TURN_LIMIT=20,MAX_MATCH_TURN_LIMIT=150,MATCH_TURN_LIMIT_STEP=10,STORE='wa-mobile-v0943',BACKUP_STORE='wa-mobile-backup-v0953';
...
function consumeTurnStartStun(k){const stunned=side(k);if((stunned.stun||0)<=0)return false;dispatchOriginalEvent('Stunned',k);const removed=stunned.inPlay.filter(e=>e.workingStiff||e.removeWhenStunned);stunned.inPlay=stunned.inPlay.filter(e=>!e.workingStiff&&!e.removeWhenStunned);stunned.discard.push(...removed.map(e=>e.card||e));for(const e of removed)addLog(`${stunned.name}'s ${e.name} leaves play when stunned.`);stunned.stun=Math.max(0,stunned.stun-1);state.control=other(k);state.position='Standing';state.message=`${stunned.name} is stunned and loses this turn.`;addLog(`${stunned.name} loses a turn while stunned${stunned.stun?` (${stunned.stun} remaining)`:''}.`);beginTurn(state.control);return true}
...
function runOriginalCardEvent(event,k,c,extraEnv={}){
  ...
  engine.WAChangeControl=who=>{const key=liveSideKey(adapter.side(who));if(key){state.control=key;side(key).lastControlTurn=state.round||0;addLog(`${side(key).name} gains control from ${c.name}.`);return true}return false};
