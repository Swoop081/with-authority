...
class WAGameStateAdapter {
  ...
  allCards(){
    ...
    for(const s of this.sides()) for(const z of ['hand','deck','discard','inPlay','connectedMoves']) for(const c of this.cards(s,z))add(c?.card||c);
    const catalogue=this.pageCatalog instanceof Map?[...this.pageCatalog.values()]:Object.values(this.pageCatalog||{});
    for(const c of catalogue)add(c);
    return out;
  }
  ...
  WAGetActiveSuperstar(who){ return this.side(who)||this.side(this.state.control)||this.state.player; }
  ...
  propertyValue(obj,key){
    ...
    const aliases={
      movetype:['moveType','originalMoveType','position'],method:['method'],momentumcost:['momentumCost','cost'],
      damage:['damage'],name:['name'],genericmomentum:['momentum.Attitude'],warnings:['warnings']
    };
    for(const path of aliases[k]||[]){let value=obj;for(const part of path.split('.'))value=value?.[part];if(value!==undefined&&value!==null&&value!=='')return value;}
    const source=obj?.sourceValues||{};for(const [name,value] of Object.entries(source))if(String(name).toLowerCase().replace(/[^a-z0-9]/g,'')===k)return value;
    return undefined;
  }
  WAGetValue(obj,key){
    ...
    if(s&&String(key)==='Generic_Momentum')return this.WAGetTotalMomentum(s);
    if(s&&String(key)==='Additional_Momentum')return Number(s.additionalMomentum)||0;
    const local=this.values(obj);if(local&&Object.prototype.hasOwnProperty.call(local,key))return local[key];
    const property=this.propertyValue(obj,key);return property===undefined?0:property;
  }
  WAHasValue(obj,key,expected){
    const value=this.WAGetValue(obj,key);
    if(arguments.length<3)return value!==0&&value!==false&&value!==null&&value!==undefined&&value!=='';
  ...
  WAGetTotalMomentum(who){ const side=this.side(who);const m=side?.momentum||{}; return Object.values(m).reduce((n,v)=>n+(Number(v)||0),0)+Math.max(0,Number(side?.additionalMomentum)||0); }
  ...
  WACanCoverCost(who,page){ const s=this.side(who); if(!s||!page)return false; const cost=Number(page.momentumCost??page.attitudeCost??page.cost??0); return this.WAGetTotalMomentum(s)>=cost; }
  ...
  WASetValue(obj,key,value){
    ...
    if(s&&String(key)==='Generic_Momentum'){s.momentum.Attitude=Math.max(0,Number(value)||0);return s.momentum.Attitude;}
    if(s&&String(key)==='Additional_Momentum'){s.additionalMomentum=Math.max(0,Number(value)||0);return s.additionalMomentum;}
  ...
  WAStringGetValue(obj,key){ const local=this.values(obj); if(local&&Object.prototype.hasOwnProperty.call(local,key))return String(local[key]); const stored=this.stringValues.get(`${this.WAGetUNID(obj)}:${key}`); if(stored!==undefined)return String(stored); const property=this.propertyValue(obj,key); return property===undefined?'':String(property); }
  ...
  WADitchFromPlaybook(who,index=0){ const s=this.side(who); if(!s||!s.deck.length)return null; const i=Math.max(0,Math.min(s.deck.length-1,Number(index)||0)); const [p]=s.deck.splice(i,1);s.discard.push(p);return p; }
  WADitchPage(who,indexOrPage){ const s=this.side(who); if(!s)return null; const i=typeof indexOrPage==='number'?Number(indexOrPage):s.hand.indexOf(indexOrPage); if(i<0||i>=s.hand.length)return null; const [p]=s.hand.splice(i,1);s.discard.push(p);return p; }
  ...
  WAAddToPageList(list,page,mode=0){
    const index=Array.isArray(list)?0:(Number(list)||0);
    const target=Array.isArray(list)?list:(this.pageLists[index]||(this.pageLists[index]=[]));
    const value=page&&typeof page==='object'?this.WAGetUNID(page):page;
    target.push(value);
    this.pageListEntries.push({list:index,page:value,mode:Number(mode)||0});
    return target;
  }
